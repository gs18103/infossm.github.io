...
- $O(n), SO(n)$: $O(n)$은 직교 행렬을 모아놓은 군입니다. 실수계수임을 강조하기 위해 $O(n,\mathbb{R})$ 등으로 적을 수도 있습니다. 두 직교 행렬의 곱이 직교행렬이고, 직교행렬의 역원은 직교행렬이며, 곱셈이나 역원 사상은 결국 다항식 꼴임을 생각하면 (역원의 경우 크라메르 공식을 생각하면 됩니다.) 매끄러운 함수임이 금방 보여 리 군임을 알 수 있습니다. $SO(n)$은 $O(n)$에서 행렬식이 1인 행렬들만 모아놓은 군입니다. $n$차원 유클리드 공간에서 거리를 보존하는 변환이 $O(n)$이며, 그중에서도 orientation을 보존하는 변환은 $SO(n)$이기 때문에 고전역학 등에서 등장합니다.
- $U(n), SU(n)$: $U(n)$은 유니터리 행렬을 모아놓은 군입니다. $O(n)$과 비슷한 이유로 리 군이 됩니다. $SU(n)$도 $SO(n)$의 정의와 비슷하게 행렬식이 1인 유니터리 행렬을 모아놓은 군입니다. 양자역학에서 복소수 위의 힐베르트 공간에 작용하는 대칭 연산을 생각할 때, 확률 보존이라는 조건 때문에 유니터리 (또는 반 유니터리) 연산자를 다뤄야 합니다. 때문에 양자역학에 나오는 대칭은 모두 유니터리 군(사실은 그 군의 표현..)으로 생각할 수 있습니다. 이외에도 양자역학에서 회전에 대한 대칭군이 $SU(2)$임은 잘 알려져 있습니다.
- $SL(n)$: 
- $O(p, q), SO(p, q)$
- $Sp(n)$
